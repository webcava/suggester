...
/**
 * Template data for Night Mode
 */
function suggester_night_mode_template() {
    return array(
        'name' => __('Night Mode', 'suggester'),
        'description' => __('Dark theme with purple accents', 'suggester'),
        'colors' => array('#121212', '#1e1e1e', '#343434', '#bb86fc'),
        'config' => array(
        ...
        'thumbnail' => 'night-mode-thumb.jpg'
    );
}

/**
 * Render Night Mode Template HTML
 * 
 * @param array $settings Tool settings
 * @return string Template HTML
 */
function suggester_render_night_mode_template($settings) {
    // Extract settings with defaults
    $config = isset($settings['template_config']) ? $settings['template_config'] : array();
    
    // Set default values if not provided
    $accent_color = isset($config['accent_color']) ? $config['accent_color'] : '#bb86fc';
    ...
    ob_start();
    ?>
    <div class="suggester-container suggester-night-mode" style="--accent-color: <?php echo esc_attr($accent_color); ?>; --button-text-color: <?php echo esc_attr($button_text_color); ?>;">
    ...
    <template id="suggester-suggestion-template">
    ...
    <template id="suggester-favorite-template">
